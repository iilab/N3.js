...
N3Store.prototype = {
      ...
      if (graphContents = graphs[graphId]) {
        ...
        if (subjectId) {
          ...
          // If subject and object are given, the object index will be the fastest
            this._findInIndex(graphContents.objects, objectId, subjectId, predicateId,
                              'object', 'subject', 'predicate', graphId, null, quads);
          else
          // If only subject and possibly predicate are given, the subject index will be the fastest
            this._findInIndex(graphContents.subjects, subjectId, predicateId, null,
                              'subject', 'predicate', 'object', graphId, null, quads);
        }
        else if (predicateId)
        // If only predicate and possibly object are given, the predicate index will be the fastest
          this._findInIndex(graphContents.predicates, predicateId, objectId, null,
                            'predicate', 'object', 'subject', graphId, null, quads);
        else if (objectId)
        // If only object is given, the object index will be the fastest
          this._findInIndex(graphContents.objects, objectId, null, null,
                            'object', 'subject', 'predicate', graphId, null, quads);
        else
        // If nothing is given, iterate subjects and predicates first
          this._findInIndex(graphContents.subjects, null, null, null,
                            'subject', 'predicate', 'object', graphId, null, quads);
      }
    }
    return quads;
    ...
    for (var graphId in graphs) {
      ...
      if (graphContents = graphs[graphId]) {
        ...
        if (subjectId) {
          if (objectId) {
          // If subject and object are given, the object index will be the fastest
            if (this._findInIndex(graphContents.objects, objectId, subjectId, predicateId,
                                  'object', 'subject', 'predicate', graphId, callback, null))
              return true;
          }
          else
            // If only subject and possibly predicate are given, the subject index will be the fastest
            if (this._findInIndex(graphContents.subjects, subjectId, predicateId, null,
                                  'subject', 'predicate', 'object', graphId, callback, null))
              return true;
        }
        else if (predicateId) {
          // If only predicate and possibly object are given, the predicate index will be the fastest
          if (this._findInIndex(graphContents.predicates, predicateId, objectId, null,
          ...
          // If only object is given, the object index will be the fastest
          if (this._findInIndex(graphContents.objects, objectId, null, null,
                                'object', 'subject', 'predicate', graphId, callback, null)) {
            return true;
          }
        }
        ...
        // If nothing is given, iterate subjects and predicates first
        if (this._findInIndex(graphContents.subjects, null, null, null,
                              'subject', 'predicate', 'object', graphId, callback, null)) {
          return true;
        }
      }
  ...
  // ### `findSubjects` returns all subjects that match the pattern.
  // Setting any field to `undefined` or `null` indicates a wildcard.
  ...
  // ### `findSubjectsByIRI` returns all subjects that match the pattern.
  // Setting any field to `undefined` or `null` indicates a wildcard.
      ...
      if (graphContents = graphs[graph]) {
        ...
        if (predicateId) {
          if (objectId)
            ...
            this._loopBy2Keys(graphContents.predicates, predicateId, objectId, handleResult);
          else
            // If only predicate is given, the SPO index is best.
            // Loop s, lookup p.
            this._loopByKey1(graphContents.subjects, predicateId, handleResult);
        }
        else if (objectId)
          // If only object is given, the OSP index is best.
          // Lookup o, loop s.
          this._loopByKey0(graphContents.objects, objectId, handleResult);
        else
          // If no params given, iterate all the subjects
          this._loop(graphContents.subjects, handleResult);
      }
    }
  },

  // ### `findPredicates` returns all predicates that match the pattern.
  // Setting any field to `undefined` or `null` indicates a wildcard.
  ...
  // ### `findPredicatesByIRI` returns all predicates that match the pattern.
  // Setting any field to `undefined` or `null` indicates a wildcard.
      ...
      if (graphContents = graphs[graph]) {
        ...
        if (subjectId) {
          if (objectId)
            ...
            this._loopBy2Keys(graphContents.objects, objectId, subjectId, handleResult);
          else
            // If only subject is given, the SPO index is best.
            // lookup s, loop p.
            this._loopByKey0(graphContents.subjects, subjectId, handleResult);
        }
        else if (objectId)
          // If only object is given, the POS index is best.
          // Loop p, lookup o.
          this._loopByKey1(graphContents.predicates, objectId, handleResult);
        else
          // If no params given, iterate all the predicates.
          this._loop(graphContents.predicates, handleResult);
      }
    }
  },

  // ### `findObjects` returns all objects that match the pattern.
  // Setting any field to `undefined` or `null` indicates a wildcard.
  ...
  // ### `findObjectsByIRI` returns all objects that match the pattern.
  // Setting any field to `undefined` or `null` indicates a wildcard.
      ...
      if (graphContents = graphs[graph]) {
        ...
        if (subjectId) {
          if (predicateId)
            ...
            this._loopBy2Keys(graphContents.subjects, subjectId, predicateId, handleResult);
          else
            // If only subject is given, the OSP index is best.
            // loop o, lookup s.
            this._loopByKey1(graphContents.objects, subjectId, handleResult);
        }
        else if (predicateId)
          // If only predicate is given, the POS index is best.
          // Looukp p, loop o.
          this._loopByKey0(graphContents.predicates, predicateId, handleResult);
        else
          // If no params given, iterate all the objects.
          this._loop(graphContents.objects, handleResult);
      }
    }
  },

  // ### `findGraphs` returns all graphs that match the pattern.
  // Setting any field to `undefined` or `null` indicates a wildcard.
  ...
  // ### `findGraphsByIRI` returns all graphs that match the pattern.
  // Setting any field to `undefined` or `null` indicates a wildcard.
